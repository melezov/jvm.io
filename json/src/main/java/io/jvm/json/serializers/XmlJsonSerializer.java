...
public class XmlJsonSerializer implements JsonSerializer<Element> {
			...
			for (final Element parentElement : parentElements) {
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (equalsWithNull(attribute.getNamespaceURI(), xmlnsURL)
							&& (equalsWithNull(resolveLocalName(attribute),xmlnsPrefix) == false)) {
						manager.addNamespace(resolveLocalName(attribute), attribute.getValue());
					}
				}
			}
		}
	 ...
	 */
	private String resolveFullName(final Node node, final XmlNamespaceManager manager){		
		
		String prefix;
		String fullName;
		
		/* If it's an xmlns node with the default xmlnsURL namespace, prefix is null, otherwise we pull it out from the namespace manager */
		if(node.getNamespaceURI() == null
				|| (equalsWithNull(resolveLocalName(node),xmlnsPrefix) && equalsWithNull(node.getNamespaceURI(),xmlnsURL)))
			prefix = null;
		else 
			prefix = manager.lookupPrefix(node.getNamespaceURI());
		
		/* If the prefix is null or empty we return just the local name, without any prepended prefix*/
	...
	/**
	 * Returns one of the string constants names for the type of the given node type. 
	 * throws IOException if type is unknown.
	 * @throws IOException 
	 */
	private String getPropertyName(final Node node, final XmlNamespaceManager manager) throws IOException {
		...
		case Node.PROCESSING_INSTRUCTION_NODE:
			return "?" + resolveFullName(node, manager);
		// XXX: The .NET System.Xml.XmlNodeType and org.w3c.dom.Node enumerations are not mapped 1:1
		...
		default:
			throw new IOException(
					"Unexpected Node when getting node name: " + node.getNodeType());
		}
	}
	
	/**
	 * Returns true if this node has the property json:Array from jsonNamespaceUri set to 'true',
	 * false otherwise
	 */
	private boolean isArray(final Node node){
		...
		if(attributes == null || attributes.getLength() == 0)
			...
			/* Return true the attribute json:Array from the jsonNamespaceUri is 'true': */
			for(int i=0; i<attributes.getLength(); i++){
				final Attr attribute = (Attr)attributes.item(i);
	...
	/**
	 * For a given root node returns a map grouping all it's children by node name; Map<nodename, nodes_having_nodename>
	 * If the node has no children, an empty map is returned.
	 * @throws IOException 
	 */
	private Map<String, List<Node>> getNodesGroupedByName(final Node root, final XmlNamespaceManager manager) throws IOException{	
		...
		for (int i = 0; i < children.getLength(); i++){
			...
			/* If the node list is not instantiated for this nodeName, make an instance */
	...
	private void serializeArray(final JsonWriter writer, final String nameOfTheElements, final List<Node> nodes, final XmlNamespaceManager manager, boolean writePropertyName) throws IOException{

        if (writePropertyName){
            writer.writeString(nameOfTheElements);
            writer.writeColon();            
        }
	...
	private void writeXmlDeclaration(final JsonWriter writer, final Document document, final XmlNamespaceManager manager) throws IOException{
		/* Deserializirat deklaraciju, iako ovo zvuči krivo staviti ovdje */
		writer.writeString(declarationNodeTag);
		writer.writeColon();
		writer.writeOpenObject();
		if(document.getXmlVersion()!=null){
			writer.writeString("@version");
			writer.writeColon();
			writer.writeString(document.getXmlVersion());
		}
		writer.writeComma();
		if(document.getXmlVersion()!=null){
			writer.writeString("@encoding");
			writer.writeColon();
			writer.writeString(document.getXmlEncoding());
		}
		writer.writeComma();
		if(document.getXmlVersion()!=null){
			writer.writeString("@standalone");
			writer.writeColon();
			writer.writeString(document.getXmlStandalone()?"yes":"no");
		}					
		writer.writeCloseObject();
	}
	
	private void serializeNode(final JsonWriter writer, final Node node, final XmlNamespaceManager manager, final boolean writePropertyName) throws IOException{
		switch(node.getNodeType()){
			...
			case Node.ELEMENT_NODE:
				if(node.hasChildNodes() && isArray(node) && allChildrenHaveSameLocalName(node))
					...
					/* Add a new namespace scope and look for namespace attributes to add  */
					manager.pushScope();
					...
					for(int i=0; i < attributes.getLength(); i++){
						...
						if(equalsWithNull(attribute.getNamespaceURI(),xmlnsURL)){
							...
							if(equalsWithNull(resolveLocalName(attribute),xmlnsPrefix))
					...
					if(writePropertyName){						
						writer.writeString(getPropertyName(node, manager));
						writer.writeColon();
					}
											
					if(hasSingleTextChild(node))											
						writer.writeString(node.getChildNodes().item(0).getNodeValue());												
						...
						for(int i=0; i<node.getAttributes().getLength(); i++)						
							serializeNode(writer, node.getAttributes().item(i), manager, true);						
				...
				break;
			case Node.COMMENT_NODE: // Different than in Json.NET, since there comments are serialized as JavaScript comments '/*...*/'
			case Node.ATTRIBUTE_NODE:
			case Node.TEXT_NODE:
			case Node.CDATA_SECTION_NODE:			
			case Node.PROCESSING_INSTRUCTION_NODE:								
			...
			// XmlDeclaration is skipped, since we are converting only xml nodes, never entire documents	
			default:
	...
	private void serializeGroupedNodes(final JsonWriter writer, final Node node, final XmlNamespaceManager manager, final boolean writePropertyName) throws IOException{
		
		final Map<String, List<Node>> nodesGroupedByName = getNodesGroupedByName(node, manager);
		
		/* Loop through grouped nodes. write single name instances as normal, write multiple names together in an array */
		for(Map.Entry<String, List<Node>> nameNodesMapping : nodesGroupedByName.entrySet()){
			...
			/* All groups of nodes are written as arrays, while single nodes are written as an array only if they have the Array attribute set */
			...
			if(!writeAsArray)
				serializeNode(writer,nodesHavingName.get(0),manager, writePropertyName);				
			else
				serializeArray(writer, name, nodesHavingName,manager, writePropertyName);
